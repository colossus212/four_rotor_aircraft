...
* and to calculate the frame rate. Units are clock cycles.  The formula is
...
*	2000		12,000		2E E0
*******************************************************************************/ 
void ADNS3080_Read_Frame_Period()
{
...
uint8_t Revision_ID, SROM_ID, ADNS3080_SROM_Download_Success = 0, SROM_CRC_Correct = 0;
...
void ADNS3080_SROM_Download()
{
	...
	SPI_Write_Byte(ADNS3080_RA_SROM_Enable, 0x18);
		//Begin burst mode write of data file to register 0x60 (SROM_Load register)
	SPI_Burst_Mode_Write(ADNS3080_RA_SROM_Load, 1986, SROM);

		// Agilent recommends reading the SROM_ID register to verify that the download was successful.
	Revision_ID = SPI_Read_Byte(ADNS3080_RA_Revision_ID);
	SROM_ID = SPI_Read_Byte(ADNS3080_RA_SROM_ID);
	if(SROM_ID == Revision_ID) ADNS3080_SROM_Download_Success = 1;

		// In addition, a self-test may be executed,
		// which performs a CRC on the SROM contents and reports the results in Data_Out register
		// SROM CRC test is initiated by writing 0xA1 to the SROM_Enable register
	SPI_Write_Byte(ADNS3080_RA_SROM_Enable, 0xa1);
		//Wait 7ms plus one frame period
	delay_us(7000 + Frame_Period);
		//then read result from the Data_Out_Lower and Data_Out_Upper registers.
