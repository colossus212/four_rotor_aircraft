...
//* Description :  http://blog.csdn.net/jia20003/article/details/7724530
//*******************************************************************************/
...
void Frame_Binaryzation()
{
	...
	for(i = 0; i < 5; i++)
		for(j = 0; j < 5; j++)
		{
			if(j > i)
			{
				( *( Get_Frame_Data_Pointer(i, j - i - 1))) = Max_1;
				( *( Get_Frame_Data_Pointer(i, 29 - (j - i - 1)))) = Max_1;
				( *( Get_Frame_Data_Pointer(29 - i, j - i - 1))) = Max_1;
				( *( Get_Frame_Data_Pointer(29 - i, 29 - (j - i - 1)))) = Max_1;
			}
		}
	...
	for(i = 0; i < 2; i++)
		for(j = 0; j < 30; j++)
		{
			( *( Get_Frame_Data_Pointer(i, j))) = Max_1;
			( *( Get_Frame_Data_Pointer(j, i))) = Max_1;
			( *( Get_Frame_Data_Pointer(29 - i, j))) = Max_1;
			( *( Get_Frame_Data_Pointer(j, 29 - i))) = Max_1;
		}
 	...
 	for(i = 0; i < 30; i++)
	{
		for(j = 0; j < 30; j++)
		{
			if(Get_Frame_Data_Matrix(i, j) >= Threshold_Line)
			{
				...
				(*(Get_Frame_Data_Pointer(i, j))) = 255;
			}
			if(Get_Frame_Data_Matrix(i, j) < Threshold_Line)
			{
...
/******************************************************************************
* function :		void Circle_Detection()
* Description : 	
*******************************************************************************/
uint8_t Circle_Centre_x = 0;
uint8_t Circle_Centre_y = 0;
uint8_t Circle_Detection_Flag = 0;
void Circle_Detection()
{
	...
	for(i = 0; i < 30; i++)
		for(j = 0; j < 30; j++)
//			if(Frame_Binaryzation_Matrix[i][j] == 255) Row[i]++;
			if((*(Get_Frame_Data_Pointer(i, j))) = 255) Row[i]++;
	for(j = 0; j < 30; j++)
		for(i = 0; i < 30; i++)
//			if(Frame_Binaryzation_Matrix[i][j] == 255) Column[j]++;
			if((*(Get_Frame_Data_Pointer(i, j))) = 255) Column[i]++;
	for(i = 0; i < 30; i++)
		if(Row[i] > Circle_Centre_y) Circle_Centre_y = i;
	for(j = 0; j < 30; j++)
		if(Column[j] > Circle_Centre_x) Circle_Centre_x = j;
}
...
float Track()
{
	...
	ADNS3080_Frame_Capture();
	Frame_Binaryzation();
	if(Circle_Detection_Flag = 1)
	{
		Circle_Detection();
		Destination_x = Circle_Centre_x;
		Destination_y = Circle_Centre_y;
	} else 
		{
			Destination_x = 0;
			Destination_y = 0;
